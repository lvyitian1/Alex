...
namespace Alex.Graphics.Models.Blocks
{
	public class CachedResourcePackModel : BlockModel
	{
		...
		private Dictionary<string, FaceCache> ProcessModelDansWay(BlockStateModel raw, out Vector3 min, out Vector3 max)
		{
			Dictionary<string, FaceCache> faceCaches = new Dictionary<string, FaceCache>();

			var rawModel       = raw.Model;
			var modelTransform = Matrix.Identity;

			float minX = float.MaxValue, minY = float.MaxValue, minZ = float.MaxValue;
			float maxX = float.MinValue, maxY = float.MinValue, maxZ = float.MinValue;

			for (var i = 0; i < rawModel.Elements.Length; i++)
			{
				var rawModelElement  = rawModel.Elements[i];
				var elementTransform = Matrix.Identity;
				
				elementTransform *= Matrix.CreateTranslation(-new Vector3(8f, 8f, 8f));

				if (rawModelElement.Rotation.Angle != 0d && rawModelElement.Rotation.Axis != Axis.Undefined)
				{

					if (rawModelElement.Rotation.Axis == Axis.X)
					{
						elementTransform *= Matrix.CreateTranslation(-rawModelElement.Rotation.Origin)
						 * Matrix.CreateRotationX(MathHelper.ToRadians((float)rawModelElement.Rotation.Angle))
							* Matrix.CreateTranslation(rawModelElement.Rotation.Origin);
					}
					else if (rawModelElement.Rotation.Axis == Axis.Y)
					{
						elementTransform *=
							Matrix.CreateRotationY(MathHelper.ToRadians((float)rawModelElement.Rotation.Angle));
					}
					else if (rawModelElement.Rotation.Axis == Axis.Z)
					{
						elementTransform *=
							Matrix.CreateRotationZ(MathHelper.ToRadians((float)rawModelElement.Rotation.Angle));
					}
				...
				elementTransform *= Matrix.CreateTranslation(new Vector3(8f, 8f, 8f));
				elementTransform *= Matrix.CreateScale(1f / 16f);

				FaceCache cache = new FaceCache();

				foreach (var face in rawModelElement.Faces)
				{
						...
						if (uv == null)
						{
							switch (face.Key)
							{
								...
								case BlockFace.South:
									x1 = rawModelElement.From.X;
									x2 = rawModelElement.To.X;
									y1 = 16f - rawModelElement.To.Y;
									y2 = 16f - rawModelElement.From.Y;
									break;
								case BlockFace.West:
								case BlockFace.East:
									x1 = rawModelElement.From.Z;
									x2 = rawModelElement.To.Z;
									y1 = 16f - rawModelElement.To.Y;
									y2 = 16f - rawModelElement.From.Y;
									break;
								case BlockFace.Down:
								case BlockFace.Up:
									x1 = rawModelElement.From.X;
									x2 = rawModelElement.To.X;
									y1 = 16f - rawModelElement.To.Z;
									y2 = 16f - rawModelElement.From.Z;
									break;
							}
						}
						else
					...
					var verts = GetFaceVertices(face.Key, rawModelElement.From, rawModelElement.To,
												GetTextureUVMap(Resources, ResolveTexture(raw, face.Value.Texture), x1,
																x2,
																y1, y2,
																face.Value.Rotation),
												out int[] indexes);
					...
					{
						var pos = verts[vi].Position;
						pos = Vector3.Transform(pos, elementTransform);
						verts[vi].Position = pos;

						minX = MathHelper.Min(minX, pos.X);
						minY = MathHelper.Min(minY, pos.Y);
						minZ = MathHelper.Min(minZ, pos.Z);
						maxX = MathHelper.Max(maxX, pos.X);
						maxY = MathHelper.Max(maxY, pos.Y);
						maxZ = MathHelper.Max(maxZ, pos.Z);
					}

					cache.Set(face.Key, new FaceData(verts, indexes, face.Value.Rotation, null));
				}

				faceCaches.Add(i.ToString(), cache);
			}
		...
		private Dictionary<string, FaceCache> ProcessModel(BlockStateModel raw, out Vector3 min, out Vector3 max) =>
			ProcessModelDansWay(raw, out min, out max);

		private Dictionary<string, FaceCache> ProcessModelKennysWay(BlockStateModel raw, out Vector3 min, out Vector3 max)
		{
		...
		protected (VertexPositionNormalTextureColor[] vertices, int[] indexes) GetVertices(
			IWorld            world,  Vector3                        position, IBlock baseBlock,
		...
		public override (VertexPositionNormalTextureColor[] vertices, int[] indexes) GetVertices(
			IWorld world, Vector3 position, IBlock baseBlock)
		{
		...
		protected class FaceData
		{
			...
			public FaceData(VertexPositionNormalTextureColor[] vertices, int[] indices, int rotation,
